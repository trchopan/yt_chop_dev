defmodule YtChopDev.DepsCustom.LangChain.ChatModels.ChatVertexAICustom do
  @moduledoc """
  ...
  defp get_api_key(%ChatVertexAICustom{api_key: api_key}) do
  ...
  def for_api(%ChatVertexAICustom{} = vertex_ai, messages, functions) do
    messages_for_api =
      messages
      |> Enum.map(&for_api/1)
      |> List.flatten()
  ...
  **NOTE:** This function *can* be used directly, but the primary interface
  ...
  def call(%ChatVertexAICustom{} = vertex_ai, prompt, tools) when is_binary(prompt) do
    messages = [
      Message.new_system!(),
  ...
  def call(%ChatVertexAICustom{} = vertex_ai, messages, tools)
  ...
  def do_api_request(%ChatVertexAICustom{stream: false} = vertex_ai, messages, tools) do
    req =
      Req.new(
  ...
  def do_api_request(%ChatVertexAICustom{stream: true} = vertex_ai, messages, tools) do
    Req.new(
      url: build_url(vertex_ai),
      json: for_api(vertex_ai, messages, tools),
      auth: {:bearer, get_api_key(vertex_ai)},
  ...
  defp build_url(%ChatVertexAICustom{endpoint: endpoint, model: model} = vertex_ai) do
    "#{endpoint}/models/#{model}:#{get_action(vertex_ai)}?key=#{get_api_key(vertex_ai)}"
    |> use_sse(vertex_ai)
  end

  @spec use_sse(String.t(), t()) :: String.t()
  defp use_sse(url, %ChatVertexAICustom{stream: true}), do: url <> "&alt=sse"
  defp use_sse(url, _model), do: url

  @spec get_action(t()) :: String.t()
  defp get_action(%ChatVertexAICustom{stream: false}), do: "generateContent"
  defp get_action(%ChatVertexAICustom{stream: true}), do: "streamGenerateContent"
  ...
  def serialize_config(%ChatVertexAICustom{} = model) do
