defmodule YtChopDev.DepsCustom.LangChain.ChatModels.ChatGoogleAICustom do
  @moduledoc """
  ...
  defp get_api_key(%__MODULE__{api_key: api_key}) do
  ...
  def for_api(%__MODULE__{} = google_ai, messages, functions) do
    messages_for_api =
      messages
      |> Enum.map(&for_api/1)
      |> List.flatten()
  ...
  **NOTE:** This function *can* be used directly, but the primary interface
  ...
  def call(%__MODULE__{} = google_ai, prompt, tools) when is_binary(prompt) do
    messages = [
      Message.new_system!(),
  ...
  def call(%__MODULE__{} = google_ai, messages, tools)
  ...
  def do_api_request(%__MODULE__{stream: false} = google_ai, messages, tools) do
    req =
      Req.new(
  ...
  def do_api_request(%__MODULE__{stream: true} = google_ai, messages, tools) do
    Req.new(
      url: build_url(google_ai),
      json: for_api(google_ai, messages, tools),
      receive_timeout: google_ai.receive_timeout
  ...
  defp build_url(
         %__MODULE__{endpoint: endpoint, api_version: api_version, model: model} = google_ai
       ) do
  ...
  defp use_sse(url, %__MODULE__{stream: true}), do: url <> "&alt=sse"
  defp use_sse(url, _model), do: url

  @spec get_action(t()) :: String.t()
  defp get_action(%__MODULE__{stream: false}), do: "generateContent"
  defp get_action(%__MODULE__{stream: true}), do: "streamGenerateContent"
